...
@section('title', 'Add Gallery Image - ' . $solution->title)

@section('content')
<div class="container-fluid px-4">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-blue-800 mb-2">Add Gallery Image</h1>
            <p class="text-gray-600">Add new image for {{ $solution->title }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        <form action="{{ route('admin.solutions.galleries.store', $solution) }}" 
              method="POST" 
            ...
            <div class="mb-8">
                ...
                <textarea id="description" 
                          ...
                          rows="4"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-300"
                          placeholder="Describe this image...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
